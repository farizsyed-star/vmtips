...
import { Trophy, Shield, Settings, LogOut, CheckCircle, BookOpen, Clock, Globe, AlertCircle, Lock, Users, Star, Sparkles, X } from "lucide-react";
...
export default function WorldCupApp() {
  ...
  const [view, setView] = useState("bonus"); // Forced to land on Bonus first
    ...
    <div className="min-h-screen bg-[#07090d] text-slate-200 font-sans pb-20">
      ...
      <nav className="max-w-5xl mx-auto px-4 mt-6 flex gap-1 bg-white/5 p-1 rounded-xl border border-white/5 overflow-x-auto scrollbar-hide">
        <button 
          ...
          className={`flex-1 min-w-[100px] py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition flex items-center justify-center gap-1 ${view === "matches" ? "bg-emerald-500 text-black" : "text-slate-500"} ${!bonusCompleted && "opacity-40 cursor-not-allowed"}`}
        >
...
function WelcomePopup({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm grid place-items-center p-6">
      <div className="bg-[#0f1117] border border-white/10 rounded-3xl p-8 max-w-md w-full relative shadow-2xl">
        <Sparkles className="text-amber-400 w-12 h-12 mb-6 mx-auto animate-pulse" />
        <h2 className="text-3xl font-black text-white text-center uppercase italic tracking-tighter mb-4">Welcome to the <span className="text-emerald-400">World Cup</span></h2>
        <div className="space-y-4 text-slate-400 text-sm leading-relaxed text-center">
          <p>Think you can outsmart your friends? Predict the scores for every match and nail the tournament bonuses to climb the leaderboard.</p>
          <p className="bg-emerald-500/10 text-emerald-400 font-bold p-3 rounded-xl border border-emerald-500/20">
            IMPORTANT: You must complete your <span className="underline">Bonus Predictions</span> first to unlock the match schedule!
          </p>
          <p>Every goal counts, every point matters. Good luck, champ!</p>
        </div>
        <button onClick={onClose} className="w-full bg-emerald-500 text-black py-4 rounded-2xl font-black uppercase mt-8 tracking-widest hover:scale-[1.02] transition-all">Let's Play</button>
      </div>
    </div>
  );
}
...
function NavBtn({ active, onClick, label }: any) {
  return (
    <button onClick={onClick} className={`flex-1 min-w-[100px] py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition ${active ? "bg-emerald-500 text-black shadow-lg shadow-emerald-500/20" : "text-slate-500 hover:text-slate-300"}`}>
      {label}
    </button>
  );
}
...
function MatchCard({ match, userId, locked, isPending }: any) {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from("predictions").upsert({ user_id: userId, match_id: match.id, pred_home: parseInt(pred.h), pred_away: parseInt(pred.a), penalty_winner_pred: pred.pw }, { onConflict: 'user_id,match_id' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/5 border border-white/10 rounded-3xl p-6 md:p-8">
        <h2 className="text-amber-400 font-black text-2xl mb-2 uppercase italic tracking-tighter">Bonus Predictions</h2>
        <p className="text-slate-500 text-xs font-bold mb-8 uppercase tracking-widest">Put your football brain to the test and predict the following:</p>
        <div className="space-y-6">
          <BonusField label="Golden Boot: Who will score the most goals?" points="5 Points" value={form.scorer} onChange={(v) => setForm({...form, scorer: v})} disabled={locked} />
          ...
          <div>
            <div className="flex justify-between items-center mb-2 ml-1">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Card Magnets: Which team will collect the most cards?</p>
              <span className="text-[9px] font-black text-emerald-400">5 Points</span>
            </div>
          ...
          <BonusField label="Goal Rush: How many goals will be scored in total?" points="5/10 Points" value={form.goals} onChange={(v) => setForm({...form, goals: v.replace(/[^0-9]/g, "")})} disabled={locked} type="number" />
          ...
          {saved && <p className="text-center text-[10px] text-emerald-400 font-black italic mt-2 uppercase">Predictions Saved! Matches unlocked.</p>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2 ml-1">
        ...
        <span className="text-[9px] font-black text-emerald-400">{points}</span>
      </div>
      <input type={type} value={value} disabled={disabled} onChange={(e) => onChange(e.target.value)} onKeyDown={(e) => type === 'number' && (e.key === '-' || e.key === 'e') && e.preventDefault()}
        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-emerald-400 outline-none font-bold italic"
      />
    </div>
  );
}

// --- REMAINING COMPONENTS (Admin, Leaderboard, etc.) ---
    ...
    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 flex flex-col items-center mb-6 shadow-lg shadow-emerald-500/5">
      ...
      {isPending ? (
        <span className="text-slate-500 font-black uppercase text-xs italic tracking-widest flex items-center gap-2">
          <Lock className="w-3 h-3" /> Awaiting Bracket Logic
        </span>
      ) : timeLeft === "LOCKED" ? (
...
function AdminPanel({ matches }: any) {
  ...
  return (
    <div className="bg-amber-400/5 border border-amber-400/20 rounded-2xl p-6">
      <h2 className="text-amber-400 font-black text-xl mb-6 flex items-center gap-2 uppercase italic underline tracking-tight italic"><Shield className="w-5 h-5" /> Admin</h2>
